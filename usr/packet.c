	...
	register struct packet *p;
{
	switch (p->p_data.pu_action.a_flag) {
		...
		case A_QUERY:
			 ...
			 * we want to send an action packet to the daemon
			 * to signify that we are, indeed, alive.
			 */
			sendaction(A_QUERY, me->u_slot, me->u_slot);
			break;
		case A_DAEMON:
			/*
